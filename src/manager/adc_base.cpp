...
void loadEnviorment( char *moduleName )
{
   ...
   sprintf( aczCompleteFileName, "%s", moduleName );
   fp = fopen( aczCompleteFileName, "r" );
   if( fp == NULL )
   {

         LOG4C(( LOG_WARN, "setenv: open file %s err %s " ,aczCompleteFileName, strerror(errno) ));
      return;
   }
